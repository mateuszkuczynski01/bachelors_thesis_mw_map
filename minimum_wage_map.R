...
minwage <- minwage %>% arrange(time) %>% filter(time>"2003-07-01"&time<"2021-01-01") %>% 
  filter(!geo %in% c("ME", "MK", "HR", "DE", "US") & currency=="EUR") %>%
...
infl <- read.csv("Inflation(2004-2019).csv")
infl <- infl[1:23,]

WB_to_EUstat <- function(x){
  if(x=="ALB"){
    return("AL")
  }else if(x=="BEL"){
    return("BE")
...
infl$Country.Code <- sapply(infl$Country.Code, WB_to_EUstat)

infl <- infl %>% select(-Series.Name, -Series.Code, -Country.Name) %>% arrange(Country.Code)  

adj_minwage <- select(mw,-geo, -`2004-01-01`) / (1+(select(infl, -Country.Code)/100))

adj_minwage <- adj_minwage %>% mutate(geo=mw$geo, `2004-01-01`=mw$`2004-01-01`, .before=1)

final_data <- adj_minwage %>% mutate(
...
plot4 <- plot3 + scale_fill_manual(name= "% increase",
                                   ...
                                   labels=c("20–22","22–45","45–48","48–120","120–552","No data"),
                                   drop=F) +
